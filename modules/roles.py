            ...
            if args[1]=='list':
                ...
                try:
                    if discord.utils.get(message.guild.roles, id=self.RoleList[args[1]][0]) in message.author.roles:
                        await message.author.remove_roles(discord.utils.get(message.guild.roles, id=self.RoleList[args[1]][0]))
                        await message.channel.send(message.author.mention + ", Vous avez perdu le rôle {args[1]}.".format())
                    else :
                ...
                try:
                    for member in self.client.get_all_members():
                        if member.id == message.author.id and discord.utils.get(member.guild.roles, id=self.RoleList[args[1]][0]) in member.guild.roles :
                            if discord.utils.get(member.guild.roles, id=self.RoleList[args[1]][0]) in member.roles:
                                await member.remove_roles(discord.utils.get(member.guild.roles, id=self.RoleList[args[1]][0]))
                                await message.channel.send(message.author.mention + ", Vous avez perdu le rôle %s."%args[1])
                            else :
