...
async def restart_py(client, message):
	if (not (message.author == client.user)) and (message.content.startswith("/restart") and await utils.perms.hasrole(message.author, settings.restart.restartAuth) and and pygit2.Repository('.').head.shorthand=="master"):
