...
class Player:
    ...
    async def send_info(self, game):
        embed = discord.Embed(
            ...
            color = 0x000055,
            description = "Il reste " + len(game["rituals"]) + " Rituels."
        )
        ...
        for id in game["order"]:
            ...
            embed.add_field(globals.number_emojis[i] + " `" + game["players"][id].user.name + "`", value, False)
            i += 1

        value = "\n".join(globals.ritual_names[x] for x in game["rituals"])
        embed.add_field("Rituels restants:", value, False)

        if game["turn"] > 0:
            embed.add_field("Carte défaussée par `" + game["players"][game["order"][game["turn"] - 1]].user.name + "`:", self.card_names[game["discard"][-1]], False)
...
class Renfield(Player):
    ...
    async def game_start(self, game):
        ...
        async def set_player(reactions):
            ...
            game["players"][index] = Vampire(game["players"][index].user)

            await self.user.send(game["players"][index].user.name + " est maitenant le Vampire")
    ...
    async def study_stack(self, game):
        ...
        if can_make_ritual:
            if len(game["rituals"]) > 1:
                player = game["players"][game["stack"][0]]
                ...
                async def silver_mirror(reactions):
                    ...
                    player = game["order"][index]

                    await self.broadcast(game, discord.Embed(
                        title = "Résultat du Miroir d'Argent",
                        color = 0x00ff00,
                        description = "Le Miroir révèle que `" + player.user.name + "` est " + ("le Vampire!" if player.role == "Vampire" else "un Chasseur")
                    ))

                    await self.check_if_stack_done(game)

                async def cond_renfield(reactions):
                    return len(reactions[self.user.id][0]) == 1
                ...
                async def holy_water(reactions):
                    ...
                    await self.broadcast(game, discord.Embed(
                        ...
                        color = 0x00ff00,
                        description = "`" + player.user.name + "` a été aspergé d'Eau Bénite, et a donc défaussé sa main. Il a pioché autant de cartes de la défausse"
                    ))

                    hand_size = len(player.hand)
                    game["discard"].extend(player.hand)
                    player.hand.clear()
                    await player.draw(game, hand_size, origin = "discard")

                    await self.check_if_stack_done(game)

                async def tranfuse(reactions):
                    index = reactions[player.user.id][0] #Le choix revient au joueur avec le Pieu
                    player = game["order"][index]
                    ...
                    await self.broadcast(game, discord.Embed(
                        ...
                        color = 0x00ff00,
                        description = "`" + player.user.name + "` a été transfusé et a donc pioché une carte" + (". Il a toujours " + player.bites + " Morsures" if player.bites else "")
                    ))

                    await player.draw(game, 1)
                ...
                async def run_ritual(reactions):
                    ...
                    if ritual == "distortion":
                        ...
                        await ReactionMessage(silver_mirror,
                            cond_renfield
                        ).send(self.user,
                            "Choisis le joueur dont le rôle sera révélé",
                            "",
                            0x00ff00,
                            [game["players"][x].user.name + " (🧛)" if game["players"].role == "Vampire" else "" for x in game["order"]]
                        )
                    elif ritual == "water":
                        await self.broadcast(game, discord.Embed(
                        ...
                        await ReactionMessage(holy_water,
                            cond
                        ).send(player.user,
                        ...
                        await ReactionMessage(transfuse,
                            cond
                        ).send(player.user,
                ...
                async def cond(reactions):
                    return len(reactions[player.user.id][0]) == 1
        ...
        if card == "bite":
            ...
            if total_bites == goal:
                ...
                async def discard(reactions):
                    card_index = reactions2[self.user.id][0]
                ...
                async def bite_player(reactions):
                    ...
                    await self.user.send("`" + player.user.name + "` a été mordu")
                    await player.user.send("Tu as été mordu! Renfield va choisir une carte de ta main pour la défausser")
                    await self.broadcast(game, discord.Embed(
                        title = "**Morsure!**",
    ...
    async def check_if_stack_done(self, game):
        if len(game["stack"]):
            await loop_through_stack(self, game)
        else:
            if game["clock"][game["turn"]] == "dawn":
                ...
                async def pass_stick(reactions):
                    index = reactions[player.user.id][0] + 1
                ...
                async def stab_player(reactions):
                    ...
                    if player:
                        ...
                        await ReactionMessage(cond,
                            ...
                            "",
                            0xffff00,
                            [game["players"][x].user.name for x in game["order"] if x != player.user.id]
                        )
    ...
    async def next_table_turn(self, game, stick_was_passed):
        ...
        if not stick_was_passed:
            ...
            await ReactionMessage(cond,
                ...
                "",
                0xffff00,
                [game["players"][x].user.name for x in game["order"] if x != game["order"][0]]
            )
        else:
            await self.turn_start(game)
    ...
    async def end_game(self, game):
        ...
        for id in game["order"]:
            ...
            embed.add_field("`" + game["players"][id].user.name + "`", value, False)
            i += 1
        ...
        del globals.games[game["channel"].id]
...
class HiddenRole(Player):
    ...
    async def send_hand(self, game):
        ...
        async def send_card(reactions):
            ...
            discard = self.hand[reactions[self.users.id][1]]

            game["stack"].append(self.hand.remove(play))
            game["discard"].append(self.hand.remove(discard))
            ...
            if clock_card == "dawn":
                ...
                [x for x in game["players"] if x.role == "Renfield"][0].study_stack(game)
...
class Vampire(HiddenRole):
    ...
    async def turn_start(self, user, game):
        await super().turn_start(game)
        ...
        await self.user.send("Il reste " + (goal - total_bites) + " Morsures à placer.")
